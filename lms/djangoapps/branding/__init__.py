...
def get_visible_courses_search(request):
    ...
    _courses = modulestore().get_courses(name=filtered_by_name)
    ...
    if filtered_by_name:
        return [course for course in courses if course.location.name == filtered_by_name]
    if filtered_visible_ids:
        return [course for course in courses if course.id in filtered_visible_ids]
    else:
        # Let's filter out any courses in an "org" that has been declared to be
        # in a Microsite
        org_filter_out_set = microsite.get_all_orgs()
        return [course for course in courses if course.location.name not in org_filter_out_set]
